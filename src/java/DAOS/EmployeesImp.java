...
public class EmployeesImp {
    ...
    public int addAdminAction(int adminId, String employeeEmail, char actionIdentifier) {
        int result = 0;
        Transaction addAdminTransaction = null;
        try {
            addAdminTransaction = employeeSession.beginTransaction();
            ...
            if (addAdminTransaction != null) {
                addAdminTransaction.commit();
    ...
    public static void main(String[] args) {
        ...
        List<Employees> allUnAssignedAdmins = EmployeesImp.getInstance().getAllUnAssignedAdmins();
        System.out.println();
    ...
    public int updateProfile(Employees updatedEmployee) {
        int result = 0;
        Transaction updateProfileTransaction = null;
        try {

            updateProfileTransaction = employeeSession.beginTransaction();
    ...
    public int assignAdmin(int adminId, int garageId) {
        Transaction assignAdminTransaction = null;
        try {
            assignAdminTransaction = employeeSession.beginTransaction();
            ...
            if (assignAdminTransaction != null) {
                assignAdminTransaction.commit();
            }
    ...
    public int updateProfileByEmail(Employees updatedEmployee) {
        int result = 0;
        Transaction updateProfile = null;
        try {

            updateProfile = employeeSession.beginTransaction();
            Query q = employeeSession.createQuery("from Employees where upper(email) like ?");
    ...
    public int deleteMemember(int memeberId) {
        ...
        int result = 0;
        Transaction deleteMemeberTransaction = null;
        try {

            deleteMemeberTransaction = employeeSession.beginTransaction();
            ...
            if (deleteMemeberTransaction != null) {
                deleteMemeberTransaction.commit();
            }
        }
        return result;
    }
    ...
    public List<Employees> getAllUnAssignedAdmins() {
        Query query = employeeSession.createQuery("from Employees e where e.roles.roleId =2 and e.garage.garageId is  null");
        return (ArrayList<Employees>) query.list();
    ...
    public List<ApplicationFeedback> getApplicationFeedback() {
        return (ArrayList<ApplicationFeedback>) employeeSession.createQuery("from ApplicationFeedback ").list();
    }

    public List<Employees> getAllEmployees(int garageId) {

        Query query = employeeSession.createQuery("from Employees where garage =:garage ");
